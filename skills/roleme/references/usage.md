...
- 如果角色已存在，直接加载
- 如果角色不存在，进入该角色的初始化流程
- 如果当前 `cwd` 本身就是 Git 仓库根目录，会自动初始化对应的 `projects/<project-slug>/`
- 自动初始化只补缺失的项目文件和索引入口，不覆盖已有项目内容
- 如果当前 `cwd` 只是仓库子目录，则不会自动创建项目目录
...
当角色已经加载后，用户不需要每次显式说“归档到哪里”或“写入哪个文件”。用户只需要自然表达内容，系统会先总结，再选择目标位置。
...
1. 项目内容优先：明显属于某个项目、仓库、需求或迭代的内容，优先写入 `projects/<project-slug>/...`
2. 稳定偏好与长期约定：明确表达“以后默认这样”“这是固定偏好/规则”的内容，写入 `memory/USER.md`
3. 长期有效的结论或方法论：从学习、复盘、实践中提炼出的可复用结论，写入 `memory/MEMORY.md`
4. 主题化知识：已经形成可反复引用的专题知识，整理到 `brain/topics/*.md` 并更新 `brain/index.md`
5. 其他情况：默认写入 `memory/episodes/`

### 默认保守策略

- 项目经验优先先落项目级位置，再判断是否值得提升为通用结论
- 不要把一次性要求或临时偏好直接写入 `memory/USER.md`
- 判断不够确定时，优先先记过程，不急着提升 resident 层

### 默认回执风格

归档完成后，助手应只用一句短回执告知结果，例如：
...
如果本次更新了 `memory/USER.md` 或 `memory/MEMORY.md`，应额外提醒用户重新执行 `/roleMe <角色名>` 以刷新当前会话底座。
