...
- P2：`doctor` 扩展，检查 memory、workflow、graph、internal skills 的健康状态
- P3：内部 skills / capability cards
- P4：session finalize / daily summary
...
Finalize 应生成一个 daily summary 文件：
...
- 初始化新角色会创建 inbox、learnings、sessions、skills index
...
- session summary 会写入 `memory/sessions/YYYY-MM-DD.md`
- session summary 会更新 `memory/sessions/index.md`
- doctor 能发现 index 指向不存在文件
...
1. P1：补目录结构与 inbox / learnings 写入 API
2. P2：扩展 doctor 检查新增结构和索引
3. P3：增加 internal skills index 与能力卡校验
4. P4：增加 session summary 写入与 finalize 数据结构
5. P5：扩展 context router，使 internal skill 与 workflow 在同一套保守路由策略下工作
